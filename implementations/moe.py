...
class Classifier(nn.Module):
    def __init__(self, transformer_dim, resnet_dim, hidden_dim=256, num_classes=1, dropout=0.1):
        ...
        self.gate = nn.Sequential(
            nn.Linear(transformer_dim + resnet_dim, hidden_dim),
            nn.Sigmoid()
        )
    ...
    def forward(self, transformer_features, resnet_features):
        ...
        fused = gate_weights * transformer_proj + (1 - gate_weights) * resnet_proj
        
        # Always use neural network for training to maintain gradients
        return self.nn_classifier(fused)
...
def train_model(training_data_folder, model_folder, sequence_length=1024, dropout=0.1, 
    ...
    with torch.no_grad():
        for ecg_data, labels in train_loader:
            ...
            # Extract features from transformer and resnet
            ...
            # Apply gated fusion to get final features
            combined = torch.cat([transformer_features, resnet_features], dim=1)
            gate_weights = model.classifier.gate(combined)
            transformer_proj = model.classifier.transformer_proj(transformer_features)
            resnet_proj = model.classifier.resnet_proj(resnet_features)
            fused = gate_weights * transformer_proj + (1 - gate_weights) * resnet_proj

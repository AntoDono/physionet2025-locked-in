...
def get_parser():
    ...
    parser.add_argument('-s', '--scp_statements_file', type=str, required=True) # scp_statements.csv
...
def run(args):
    ...
    agg_df = pd.read_csv(args.scp_statements_file, index_col=0)
    agg_df = agg_df[agg_df.diagnostic == 1]
    
    # Function to aggregate diagnostic codes to superclass
    ...
    for record in tqdm.tqdm(records):
        ...
        # Determine label based on diagnostic superclass
        # NORM = 0, any other superclass = 1
        ...
        if os.path.normpath(args.input_folder) != os.path.normpath(args.output_folder):
            ...
            for input_signal_file in signal_files:
                signal_basename = os.path.basename(input_signal_file)
                output_signal_file = os.path.join(args.output_folder, signal_basename)
        ...
        if args.signal_format in ('mat', '.mat'):
            convert_dat_to_mat(os.path.join(args.output_folder, record_basename), write_dir=args.output_folder)
